...
is considered to be  negative 1. In case of a tie between several elements in choices, the one with the smallest index
...
for i in dictionary_count(word):
    for m in dictionary_count(
            choices[0]):  # this function will have 2 word/strings as inputs, latter to replace choices[0]
        if i == m:
