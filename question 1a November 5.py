    ...
    for word in dictionary_count: #each new/inserted word in the dict
        for sentence in sentences:
            for new_word in sentence:
                if (new_word != word) and (new_word not in word): #not sure about first past since new_word is a word and word is a dictionary, but make sure we're comparing words, not using the same ones
                    word[new_word] = 1
